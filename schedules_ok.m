...
airplane_input = [n1 n2 n3]; % n6 n7 n8 n9 n10]% n11 n12 n13];
...
for i = 1:airplane
    ...
    for j = 1:c
       ...
       % [error_dx, error_dy, error_dz, error_spd, error_vsp] = rpos(pos_acc, alt_acc, spd_acc, vsp_acc);
            
        Route{1,i}(j,1) = NavData(NavAIP{1,a}(j,1),1);%+ error_dx;      % X
        Route{1,i}(j,2) = NavData(NavAIP{1,a}(j,1),2);%+ error_dy;      % Y
        Route{1,i}(j,3) = NavData(NavAIP{1,a}(j,1),2+m);%+ error_dz;    % Z(Altitude) 
        Route{1,i}(j,4) = NavData(NavAIP{1,a}(j,1),7+m);% + error_spd;    % Lateral Speed 
        Route{1,i}(j,5) = NavData(NavAIP{1,a}(j,1),12+m);% + error_vsp ;   % Vertical Speed
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        if b == NavAIP{2,a}(1,1)
                            a = a+1;
